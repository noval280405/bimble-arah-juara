...
<template>
  ...
  >
    ...
    <h3>{{ program.name }}</h3>
    <p class="mb-6 mt-3 text-sm">{{ program.description }}</p>
    <NuxtLink
      :to="'/program/' + program.slug"
      class="mt-auto border-t border-slate-100 pt-4 text-sm font-bold text-blue-600"
      >Lihat Program <span class="float-right">↗</span></NuxtLink
    >
  </article>
</template>
